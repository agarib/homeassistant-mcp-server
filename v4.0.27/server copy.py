...
✅ System Diagnostics (6 tools) - Logs, notifications, integration health, startup errors, config check, system health
...
✅ File Operations (10 tools) - Full /config access, search, tree, copy, move, LIST FILES
...
✅ System (3 tools) - Restart HA (+ alias), call service
...
v4.0.14 (2025-11-13):
  ...
  📊 ENDPOINTS: 95 → 97 endpoints (System Diagnostics: 4 → 6 tools)
  🎯 IMPACT: Complete Cloud AI compatibility - all expected endpoints now available

v4.0.13 (2025-11-13):
  🐛 FIX: ha_create_automation validation improved
  ✅ FIXED: Added min_length=1 validation for trigger/action arrays
  📝 REASON: Prevents creation of invalid automations with empty trigger/action lists
  ⚡ IMPACT: Cloud AI will get proper 422 errors for invalid automation requests

v4.0.12 (2025-11-13):
  🐛 FIX: Pydantic V2 deprecation warning resolved
  ✅ FIXED: Changed Field(example=...) → Field(json_schema_extra={"example": ...})
  📝 REASON: Pydantic V2.0+ requires json_schema_extra for OpenAPI examples
  ⚡ IMPACT: Removes deprecation warning on startup

v4.0.11 (2025-11-13):
  🐛 CRITICAL FIX: Added /ha_restart alias endpoint for Cloud AI compatibility
  ✅ FIXED: Cloud AI agents can now call /ha_restart (maps to /ha_restart_homeassistant)
  📝 REASON: Cloud AI was hanging on missing /ha_restart endpoint
  🔄 ENDPOINTS: 98 → 99 endpoints (System tools: 2 → 3)
  ⚡ IMPACT: Resolves Cloud AI automation workflow blocking issue

v4.0.10 (2025-11-13):
  🐛 CRITICAL FIXES: Multiple production issues resolved
  ✅ FIXED: Added missing ha_list_files endpoint (404 errors resolved)
  ✅ FIXED: ha_get_automation_details - Changed get_state() → get_states() (AttributeError fixed)
  ✅ FIXED: ha_search_files - Added regex validation and auto-escaping for special characters
...
v4.0.9 (2025-11-12):
  🐛 CRITICAL FIX: ha_reload_automations now works correctly
  ✅ FIXED: Added missing ReloadAutomationsRequest Pydantic model (Line ~1305)
  ✅ FIXED: Changed get_states("automation") → get all states + filter by domain (Line 5014)
  🎯 Result: Automation reload from YAML files now functional
  🎯 Result: Resolves "Entity not found" error when reloading automations
  📝 Impact: Automation development workflow fully restored

v4.0.8 (2025-11-11):
  🐛 CRITICAL FIX: ha_process_intent endpoint path corrected
  ✅ FIXED: Changed /api/conversation/process → /conversation/process (supervisor proxy)
  ✅ VERIFIED: ha_render_template working correctly (returns response.text.strip('"'))
  ✅ VERIFIED: ha_control_light working correctly (uses request.action field)
  🎯 Result: Voice intent processing now works with Extended OpenAI Conversation
  🎯 Result: Resolves double /api/api path issue (supervisor proxy includes /api)
  🎯 Result: All 97 tools maintain 100% availability
  📝 Impact: Voice assistant integration fully functional with Open-WebUI
...
# Try to get token from environment first, then from s6-overlay store
HA_TOKEN = os.getenv("SUPERVISOR_TOKEN", "")
SUPERVISOR_TOKEN = os.getenv("SUPERVISOR_TOKEN", "")

# If token is missing, try reading from s6-overlay environment store
...
class HomeAssistantAPI:
    ...
    async def render_template(self, template: str) -> str:
        ...
        response.raise_for_status()
        # HA returns the rendered template as plain text in the response body
        return response.text.strip('"')  # Strip quotes if JSON-encoded string
...
@app.post("/ha_list_areas", operation_id="ha_list_areas", summary="List all areas", tags=["discovery"])
async def ha_list_areas(request: ListAreasRequest = Body(...)):
    """
    List all areas in Home Assistant.
    """
    try:
        # Use WebSocket API as REST API doesn't expose registries
        ws_client = await get_ws_client()
        areas = await ws_client.call_command("config/area_registry/list")
...
@app.post("/ha_list_devices", operation_id="ha_list_devices", summary="List all devices", tags=["discovery"])
async def ha_list_devices(request: ListDevicesRequest = Body(...)):
    """
    List all devices in Home Assistant registry.
    """
    try:
        # Use WebSocket API as REST API doesn't expose registries
        ws_client = await get_ws_client()
        devices = await ws_client.call_command("config/device_registry/list")
...
class ListFilesRequest(BaseModel):
    ...
    extensions: Optional[List[str]] = Field(None, description="Filter by file extensions (e.g., ['yaml', 'py'])")
        ...
        for file_path in search_path.rglob("*"):
            ...
            try:
                async with aiofiles.open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    # Escape regex special characters if pattern looks like plain text
...
@app.post("/ha_list_files", operation_id="ha_list_files", summary="List files in a directory", tags=["files"])
async def ha_list_files(request: ListFilesRequest = Body(...)):
    """
    List all files in a directory, optionally filtered by extension.
    
    Example: {"directory": "packages", "extensions": ["yaml"], "recursive": true}
    """
    try:
        search_path = file_mgr.ha_resolve_path(request.directory)
        files = []
        ...
        if request.recursive:
            # Recursive search
            pattern = "**/*" if not request.extensions else f"**/*.{{{','.join(request.extensions)}}}"
            for file_path in search_path.glob(pattern):
                if file_path.is_file():
                    # Filter by extension if specified
                    if request.extensions and file_path.suffix.lstrip('.') not in request.extensions:
                        continue
                    
                    files.append({
                        "name": file_path.name,
                        "path": str(file_path.relative_to(HA_CONFIG_PATH)),
                        "size": file_path.stat().st_size,
                        "extension": file_path.suffix.lstrip('.')
                    })
        else:
            # Non-recursive (immediate directory only)
        ...
        return SuccessResponse(
            message=f"Found {len(files)} files in {request.directory}",
...
class CreateAutomationRequest(BaseModel):
    alias: str = Field(..., description="Automation friendly name", min_length=1)
    trigger: List[Dict[str, Any]] = Field(..., description="List of trigger configurations", min_length=1)
    condition: Optional[List[Dict[str, Any]]] = Field(None, description="Optional list of conditions")
    action: List[Dict[str, Any]] = Field(..., description="List of actions to execute", min_length=1)
...
class ReloadAutomationsRequest(BaseModel):
    validate_only: Optional[bool] = Field(False, description="Only validate YAML without reloading")
...
@app.post("/ha_restart", operation_id="ha_restart", summary="Restart Home Assistant (Alias)", tags=["system"])
async def ha_restart(request: RestartHomeAssistantRequest = Body(...)):
    """
    🔄 ALIAS ENDPOINT for /ha_restart_homeassistant
    
    This endpoint exists for Cloud AI compatibility. Cloud AI agents expect
    the shorter endpoint name /ha_restart instead of /ha_restart_homeassistant.
    
    This is a direct alias that calls the same function with identical behavior.
    See /ha_restart_homeassistant documentation for full details.
    ...
    return await ha_restart_homeassistant(request)
...
class GetEntityHistoryRequest(BaseModel):
    ...
    start_time: Optional[str] = Field(None, description="Start time (ISO format or relative like '-25h')")
...
async def ha_get_entity_history(request: GetEntityHistoryRequest = Body(...)):
    ...
    **USE CASES:**
    ...
    Example: {"entity_id": "light.living_room", "start_time": "-25h"}
    """
    try:
...
async def ha_get_statistics(request: GetStatisticsRequest = Body(...)):
    ...
    **USE CASES:**
    ...
    try:
        ...
        start_time = end_time - timedelta(hours=period_hours.get(request.period, 25))
...
class CreateMiniGraphCardRequest(BaseModel):
    ...
    name: Optional[str] = Field(None, description="Card name")
    hours_to_show: int = Field(25, description="Hours of history to show")
...
class IntelligentSecurityMonitorRequest(BaseModel):
    ...
    baseline_hours: int = Field(25, description="Hours of history to establish baseline")
...
class ProcessIntentRequest(BaseModel):
    text: str = Field(..., description="Natural language command", json_schema_extra={"example": "turn on the kitchen lights"})
...
async def ha_process_intent(request: ProcessIntentRequest = Body(...)):
    """
    Process natural language commands using Home Assistant's built-in Conversation/Assist API.
    Converts text like "turn on the kitchen lights" into actions.
    
    Uses official /conversation/process endpoint (via supervisor proxy at /core/api).
    ...
    Returns:
    - speech: TTS response text
    - response: Intent processing result
    - conversation_id: For follow-up queries
    """
    try:
        response = await http_client.post(
            f"{HA_URL}/conversation/process",
            json={
                "text": request.text,
        ...
        return SuccessResponse(
            ...
            data={
                ...
                "speech": intent_result.get("response", {}).get("speech", {}).get("plain", {}).get("speech", ""),
                "conversation_id": intent_result.get("conversation_id"),
                "source": "Conversation API"
            }
        )
    except httpx.HTTPStatusError as e:
        logger.error(f"Conversation API error: {e.response.status_code} - {e.response.text}")
        raise HTTPException(
            status_code=e.response.status_code,
            detail=f"Conversation API error: {e.response.text}"
        )
    except Exception as e:
...
# Check Config (Alias for ha_validate_config)
@app.post("/ha_check_config", operation_id="ha_check_config", summary="Check HA configuration (Alias)", tags=["advanced_api"])
async def ha_check_config():
    """
    🔄 ALIAS ENDPOINT for /ha_validate_config
    
    This endpoint exists for Cloud AI compatibility. Cloud AI agents expect
    /ha_check_config instead of /ha_validate_config.
    
    This is a direct alias that calls the same function with identical behavior.
    See /ha_validate_config documentation for full details.
    """
    return await ha_validate_config()


# System Health
@app.post("/ha_system_health", operation_id="ha_system_health", summary="Get system health status", tags=["system_diagnostics"])
async def ha_system_health():
    """
    Get comprehensive system health information including:
    - Component status (recorder, logger, etc.)
    - Integration health
    - System resources
    - Database size
    - Last boot time
    
    Example response includes status for all loaded components.
    """
    try:
        # Try the official system health API endpoint first
        try:
            response = await http_client.get(f"{HA_URL}/system_health/info")
            response.raise_for_status()
            ...
            return SuccessResponse(
                ...
                data={
                    "health": health_data,
                    "source": "System Health API"
                }
            )
        except httpx.HTTPStatusError:
            # Fallback: gather health from various endpoints
            config = await ha_api.get_config()
            states = await ha_api.get_states()
            
            return SuccessResponse(
                message="System health retrieved (basic)",
                data={
...
if __name__ == "__main__":
    logger.info("🚀 Starting Home Assistant OpenAPI Server v4.0.28")
