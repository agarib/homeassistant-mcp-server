...
class FileManager:
    ...
    def ha_resolve_path(self, filepath: str) -> Path:
        ...
        # Also remove /config prefix if present (users sometimes include it)
        clean_path = filepath.lstrip("/\\")
        if clean_path.startswith("config/") or clean_path.startswith("config\\"):
            clean_path = clean_path[7:]  # Remove 'config/' or 'config\\'
        path = (self.base_path / clean_path).resolve()
